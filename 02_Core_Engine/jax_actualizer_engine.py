...
class JaxActualizerEngine:
    ...
    # ---- Internal: builds the jitted function (closure over hyperparams) --

    def _build_steer_fn(self):
        V, k, Q_c, tau, tau_bif, max_iters = (
            self.V, self.k, self.Q_c, self.tau, self.tau_bif, self.max_iters
        )
        w_L, w_G, w_F = self.w_L, self.w_G, self.w_F
        ...
        def drift_tensor(U, hist_ids, hist_valid, hist_stepback, target_mask):
        ...
        def body_fn(state, hist_ids, hist_valid, hist_stepback, target_mask):
            ...
            D = drift_tensor(U, hist_ids, hist_valid, hist_stepback, target_mask)
            Tr_D = jnp.dot(U, D)
        ...
        def steer_fn(logits, target_mask, hist_ids, hist_valid, hist_stepback):
            U0 = softmax(logits)
            init_state = (0, U0, U0, False, 0.0)

            def body(state):
                return body_fn(state, hist_ids, hist_valid, hist_stepback, target_mask)
