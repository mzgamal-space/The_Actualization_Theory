...
class ActualizerStageResult:
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"status={status}, ν={self.final_nu:.4f}, "
...
class ActualizerFDSAQCAPipeline:
    ...
    def _run_qca_stage(
        ...
        def _steer_cluster(cluster: QCACluster, pruned_logits_np: np.ndarray):
            ...
            for n in cluster.nodes:
                ...
                # Generate node logits: same seeded approach as qca_parallel_engine worker
                node_rng    = random.Random(n.node_id * 1000 + int(base_val * 100))
                # LLM-realistic: wide background + a few peaks
                logits_list = [node_rng.gauss(-12.0, 3.0) for _ in range(self.cfg.vocab_size)]
                n_cand      = max(3, self.cfg.vocab_size // 20)
                for _ in range(n_cand):
                    logits_list[node_rng.randint(0, self.cfg.vocab_size - 1)] = node_rng.gauss(1.5, 2.0)
                for _ in range(node_rng.randint(1, 3)):
                    logits_list[node_rng.randint(0, self.cfg.vocab_size - 1)] = node_rng.uniform(5.0, 9.0)

                logits_np = np.array(logits_list, dtype=np.float64)

                target_center = int((prime_prof[0] if prime_prof else 0.5) * self.cfg.vocab_size) % self.cfg.vocab_size
                target_toks   = set(range(max(0, target_center - 20), min(self.cfg.vocab_size, target_center + 20)))
                history       = [max(0, target_center - 1)]

                # FDSA prune
                pruned_np, active = self._qca_engine.pruner.prune_numpy(
                    logits=logits_np,
                ...
                # Early exit: if FDSA pruned everything, emit a dummy vote (no wasted Actualizer work)
                if active == 0:
                ...
                # Actualizer steer — capped at _CLUSTER_MAX_ITERS for speed
    ...
    def run(
        ...
        # ── Derive target_tokens from active vocab if not provided ────────────
        if target_tokens is None:
            # Top-K active tokens by raw logit score become targets
            active_indices = [
                i for i, v in enumerate(fdsa_result.pruned_logits)
                if not math.isinf(v) and not math.isnan(v)
            ]
            target_tokens = set(active_indices[:50]) if active_indices else set(range(self.cfg.vocab_size))

        # ── Stage 2 & 3: Sequential or Parallel execution ────────────────────
        qca_result: Optional[QCAParallelResult] = None

        if self.cfg.execution_mode == "parallel" and self._qca_engine is not None:
            # --- PARALLEL PATH ---
            # N_nodes = K*2+1: minimum viable for clustering that gives K non-empty clusters.
            # Fewer nodes = less per-cluster Actualizer work = faster parallel wall-clock.
            N_nodes = self.cfg.K * 2 + 1
            nodes = self._build_qca_nodes(fdsa_result.pruned_logits, context_ids, N_nodes)
            log.append(f"[Stage 2 — QCA] Building {len(nodes)} nodes from {fdsa_result.active_count} active tokens")

            qca_result, meta_logits = self._run_qca_stage(
                nodes,
                np.array(fdsa_result.pruned_logits, dtype=np.float64),
                log,
            )

            # Combine meta-logits with FDSA-pruned logits for final snap
            # Meta-logits carry cluster-level consensus; FDSA pruning gate is preserved
            final_logits = [
                (fdsa_result.pruned_logits[i] + meta_logits[i])
                if not math.isinf(fdsa_result.pruned_logits[i]) else -math.inf
                for i in range(self.cfg.vocab_size)
            ]
        else:
            # --- SEQUENTIAL PATH ---
            log.append("[Stage 2 — QCA] Skipped (execution_mode=sequential)")
            final_logits = fdsa_result.pruned_logits

        # ── Stage 3: Final Actualizer Snap ────────────────────────────────────
        act_result = self._run_actualizer_stage(
            logits_to_steer=final_logits,
